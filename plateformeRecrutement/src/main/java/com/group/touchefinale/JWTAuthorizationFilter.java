...
public class JWTAuthorizationFilter extends OncePerRequestFilter 
{
	  ...
	  if(request.getMethod().equals("OPTIONS")) {
		  ...
		  if (jwt==null || !jwt.startsWith(SecurityConstants.TOKEN_PREFIX)) {
			  ...
			  // deserialiser les roles ou niveau dacces
			  Claims claims=Jwts.parser().setSigningKey(SecurityConstants.SECRET).parseClaimsJws(jwt.replace(SecurityConstants.TOKEN_PREFIX, ""))
					  .getBody();
			  String mailutilisateur=claims.getSubject(); 
			  ...
			  // verification du token et integration dans le contexte de securité de spring security 
			  UsernamePasswordAuthenticationToken authenticatedUser= new
			  UsernamePasswordAuthenticationToken(mailutilisateur, null, authorities);
